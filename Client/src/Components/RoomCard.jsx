...
const RoomCard = ({ room, onSelect }) => {
  ...
  return (
    <div className="bg-gray-100 rounded-xl overflow-hidden border border-gray-200 shadow-lg hover:shadow-xl transition-all duration-300 h-auto">
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-1 p-4 sm:p-6 md:p-8 flex flex-col justify-between">
          <div>
            <h3 className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-2 sm:mb-3">{roomName}</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-3 sm:mb-4">
              <div className="bg-gray-200 p-2 sm:p-3 rounded-lg text-center">
                <p className="text-gray-600 text-xs sm:text-sm mb-1">Size</p>
                <p className="font-semibold text-xs sm:text-sm">{size} m²</p>
              </div>
              <div className="bg-gray-200 p-2 sm:p-3 rounded-lg text-center">
                <p className="text-gray-600 text-xs sm:text-sm mb-1">Bed</p>
                <p className="font-semibold text-xs sm:text-sm">{bedType}</p>
              </div>
              <div className="bg-gray-200 p-2 sm:p-3 rounded-lg text-center">
                <p className="text-gray-600 text-xs sm:text-sm mb-1">Adults</p>
                <p className="font-semibold text-xs sm:text-sm">{adults}</p>
              </div>
              <div className="bg-gray-200 p-2 sm:p-3 rounded-lg text-center">
                <p className="text-gray-600 text-xs sm:text-sm mb-1">Children</p>
            ...
            {transportations && transportations.length > 0 && (
              <div className="mb-2 sm:mb-3">
                {transportations.map((t, idx) => (
                  <div key={idx} className="flex items-center gap-2 bg-gray-300 text-gray-700 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm w-fit mb-1">
            ...
            <div className="flex flex-wrap gap-2 mb-2 sm:mb-3">
              {amenities.map((amenity, idx) => (
                <span
                  key={idx}
                  className="bg-gray-300 text-gray-700 px-2 sm:px-3 py-0.5 sm:py-1 rounded-md text-xs sm:text-sm"
                >
                  {amenity}
                </span>
              ))}
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-t border-gray-200 pt-3 sm:pt-4 mt-3 sm:mt-0">
            <div className="mb-6 sm:mb-0">
              <p className="text-gray-600 text-xs sm:text-sm">${currentPrice} × {nights} {nights === 1 ? 'night' : 'nights'}</p>
            ...
            <button
              ...
              className="bg-lapis_lazuli-500 hover:bg-indigo_dye-500 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-lg flex items-center gap-2 transition-colors shadow-md hover:shadow-lg w-full sm:w-auto text-xs sm:text-sm min-h-[44px]"
            >
